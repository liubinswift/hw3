...
//  }
  ...
  public static String[] getSiteInfoByCode(String siteCode) {
    ...
    String taskCode =
        ...
        "manufacturer,ip  from res_headend_tab where is_delete=0 and upper(code) like '%" +
...
//  }
  ...
  public static String[] getSiteFreqs(String siteCode) throws
    ...
    if (siteCode != null && siteCode.length() > 0) {
      ...
      try {
        ...
        throw new Exception("查询频率信息出错！", ex);
      }
   ...
   * @throws java.lang.Exception
   */
  public static String getSiteHeadIdsByCodeNoAB(String siteCode) {
    String siteIds = "";
    String taskCode =
        "select version,type_id,shortname,head_id,code site_code," +
        "manufacturer,ip  from res_headend_tab where is_delete=0 and upper(code)  like '" +siteCode.toUpperCase()+"%'";
    GDSet resultCode = null;
    //大写代码与详细信息的对照。
    
      try {
		resultCode = DbComponent.Query(taskCode);
		for(int i=0;i<resultCode.getRowCount();i++)
		{
			siteIds+=resultCode.getString(i, "head_id")+",";
		}
	} catch (DbException e) {
